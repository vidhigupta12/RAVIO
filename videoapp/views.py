...
from .sentiments import naive_bayes_predict, get_loglikelihood, get_logprior
from videoapp import sentiments
import re
...
def get_sentiments(comment):
    ...
    x = naive_bayes_predict(comment, logprior, loglikelihood)
    if x > 0:
        return 1
    else:
        return -1
...
def add_comment(request):
    if request.method == 'GET':
        ...
        str_comment = comment
        str(str_comment)
        sentiments = get_sentiments(str_comment)
        video_obj = VideoPost.objects.get(id=video_id)
        session_obj = User.objects.get(username=request.user.username)
        if(sentiments == 1):
            video_obj.positive_comment += 1
        else:
            video_obj.negative_comment += 1
        
        video_obj.save()
        video_comments = Comment.objects.filter(post=video_obj).order_by('-id')
        create_comment = Comment.objects.create(post=video_obj, user=session_obj, comment=comment, sentiment = sentiments)
        create_comment.save()
...
def get_hashtag(request):
    ...
    for video in videos:
        hashtags = hashtag_extract(str(video.desc))
        print(hashtags)
        for h in hashtags:
    ...
    all_hashtags = dict(sorted(all_hashtags.items(), key=lambda item: item[1], reverse = True))
    lst = []
